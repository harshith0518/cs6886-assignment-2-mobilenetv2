...
def figures(result, folder):
    ...
    fig.text(0.055, 0.905, "Local companion figure. The required native W&B panel is prepared separately by report_q3.py.", fontsize=10, color="#465366")
...
def log_wandb(result, folder, mode, project, entity):
    ...
    report = wr.Report.from_url(manifest["report_url"]) if manifest.get("report_url") else wr.Report(
    ...
    report.save()
    manifest.update(report_url=report.url, project_url=f"https://wandb.ai/{entity}/{project}",
                    native_chart_published=True, uploaded_metrics_verified=True)
    save_json(manifest_path, manifest)
    restored = wr.Report.from_url(report.url)
    ...
    save_json(manifest_path, manifest)
    print(f"Verified W&B report: {report.url}", flush=True)
