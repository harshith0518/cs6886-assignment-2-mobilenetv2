"""Manual weight and activation quantization, with a packed model file format."""
import copy
import json
import math
import struct
...
def quantize_weight(weight, bits):
    # Use a separate scale for each output channel, including depthwise channels.
    check_bits(bits)
    if bits == 32:
...
def quantize_activation(values, bits, scale, zero_point):
    # Round to the calibrated grid, then convert back to floats for inference.
    check_bits(bits)
    if bits == 32:
        return values
