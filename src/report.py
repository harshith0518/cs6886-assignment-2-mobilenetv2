...
from src.train import ROOT, RESULTS, baseline_files, save_json
...
def verify_results(folder, run_dir=None, check_files=False):
    checkpoint_path, baseline_results_path, splits_path = baseline_files(run_dir)
    raw = (folder / "compression.json").read_bytes()
    ...
    with np.load(folder / "compression_indices.npz") as indices, np.load(splits_path) as splits:
    ...
    for row in rows:
        ...
        if folder == DEFAULT_RESULTS and not packed.exists():
            packed = ROOT / "models" / packed.name
        if packed.exists():
...
def native_panel():
    ...
    return wr.ParallelCoordinatesPlot(
        ...
        gradient=[wr.GradientPoint(color="#111111", offset=0), wr.GradientPoint(color="#777777", offset=50),
                  wr.GradientPoint(color="#bbbbbb", offset=100)],
    )
...
def log_wandb(result, folder, mode, project, entity):
    ...
    manifest_path = folder / ("wandb.json" if mode == "online" else "wandb_offline.json")
    ...
    if mode == "offline":
        ...
        manifest["next_step"] = "Login, then run python -m src.report --wandb-mode online. No model experiments need rerunning."
        save_json(manifest_path, manifest)
        return
...
def write_answer(result, folder):
    ...
    manifest = folder / "wandb.json"
    if manifest.exists():
        url = json.loads(manifest.read_text()).get("report_url")
        if url:
            lines += [f"[Native W&B Parallel Coordinates chart]({url})", ""]
    lines += ["The repository keeps the baseline and W4A8. The README sweep command regenerates all nine models.", "",
...
def main():
    ...
    result = verify_results(folder, args.run_dir, args.check_files)
